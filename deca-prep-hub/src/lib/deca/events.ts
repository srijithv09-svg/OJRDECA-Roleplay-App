...
export function detectDecaEventFromResource(resource: Partial<ResourceListItem>) {
  ...
  const detectedCode = detectDecaEventCodeFromText(resourceDetectionText(resource));

  return detectedCode ? getDecaEventByCode(detectedCode) : null;
}

export function detectDecaEventFromFilename(filename: string) {
  const detectedCode = detectDecaEventCodeFromText(filename);
