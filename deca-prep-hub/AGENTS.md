...
- Phase 8 adds readiness dashboards by aggregating existing data only. Student `/dashboard` should guide the next action across MCS learning, concept feedback revisions, roleplay practice, and exam practice. Student `/analytics` is the deeper history/trend view. Admin `/admin/analytics` should show chapter activity, weak MCS concepts, roleplay feedback trends, exam progress, and content review queues.
- Phase 8 readiness data is served through authenticated API routes and server helpers. Do not expose service role keys in client code. Optional sections must degrade independently with local unavailable states; a failed roleplay, exam, learning, or review-queue query should not crash the entire dashboard.
- Phase 9 adds `/admin/content` as the Learning Content Studio. Keep it inside the Admin workspace, not the top-level sidebar. Admins/advisors can manage modules/key sets, concepts, questions, and study resources; students only see approved content through `/learn`.
- Phase 9 introduces `study_resources` for approved supplemental learning links/notes/resources. Apply `20260616150000_add_study_resources.sql` before relying on `/admin/content`, student study resource displays, or `npm run check:db`.
- Phase 9.5 adds PI-grounded curriculum drafting inside `/admin/content`. It uses selected approved `roleplay_performance_indicators` rows or a manually pasted PI list as the source of truth, then drafts modules/key sets, concepts, and aligned questions through server-side Gemini only. It is not a generic question generator.
- Phase 9.5 draft output is stored as `draft` modules/concepts and `needs_review` questions with source PI metadata and optional `curriculum_draft_jobs` audit records. Admins/advisors must review, edit, and approve content before students can see it; AI drafts must never auto-approve or publish.
- Phase 9.5 generation is capped by PI count, module count, and questions per concept. Missing-key, quota, timeout, or invalid-output failures should return clean admin-facing errors. Avoid copying exam/roleplay wording; use original learning content grounded in short performance-indicator labels.
- Phase 9.25 adds `profiles.selected_cluster` for student personalization. Allowed values are `entrepreneurship`, `marketing`, `business_management_administration`, `hospitality_tourism`, and `finance`. This preference only changes dashboard recommendations and learning/resource ordering; it must never restrict access to approved modules, resources, roleplays, exams, or study resources outside the selected cluster.
- Cluster-aware recommendations should fall back gracefully to the active MCS guided pathway when a selected cluster does not yet have approved learning content. `/learn` and library pages should keep all approved content reachable and make clear that cluster preference changes order, not access.
